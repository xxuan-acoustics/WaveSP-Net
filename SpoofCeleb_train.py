...
import config_spoofceleb
torch.set_default_tensor_type(torch.FloatTensor)
torch.multiprocessing.set_start_method('spawn', force=True)

def initParams():
    parser = config_spoofceleb.initParams()
...
def train(args):
    ...
    if feat_model is None:
        raise RuntimeError("Model initialization failed. feat_model is still None after all conditions.")      
    ...
    spoofceleb_trainset = spoofceleb(args.spoofceleb_train_audio,
                                     args.spoofceleb_train_label,
                                     audio_length=args.audio_len,
                                     rawboost=False,
                                     rawboost_log=args.rawboost_log)
    
    spoofceleb_devset = spoofceleb(args.spoofceleb_dev_audio,
                                   args.spoofceleb_dev_label,
                                   audio_length=args.audio_len,
                                   rawboost=False,
                                   rawboost_log=args.rawboost_log)
    ...
    if args.train_task == "speech":
        train_set = [spoofceleb_trainset]
        dev_set = [spoofceleb_devset]
