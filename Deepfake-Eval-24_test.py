...
def init():
    ...
    parser.add_argument("--df24_eval_audio", type=str,
                        default='/home/xxuan/speech-deepfake/datasets/DF24Datasets/test_audio')
    parser.add_argument("--df24_eval_label", type=str,
...
def DF24_test_on_speech(model, args, model_name="", optimal_threshold=0.5):

    results_dir = os.path.join(args.model_path, 'results')
    os.makedirs(results_dir, exist_ok=True)

    suffix = model_name.replace("_model", "")
    file_name = f'DF24_{suffix}_speech.txt' if suffix else 'DF24_speech.txt'
    file_path = os.path.join(results_dir, file_name)

    df24_testset = DF24Dataset(
        args.df24_eval_audio,
    ...
    test_loader = DataLoader(df24_testset, batch_size=args.batch_size, shuffle=False, num_workers=8)
    ...
    y_pred = (np.array(y_score_all) >= optimal_threshold).astype(int)
    ...
    print(f"[DF24] Scores saved to: {file_path}")
    print(f"[DF24] Metrics saved to: {acc_path}")
...
if __name__ == "__main__":
    ...
    for key, filename in model_files.items():
        ...
        feat_model.eval()
        DF24_test_on_speech(feat_model, args, model_name=key)
